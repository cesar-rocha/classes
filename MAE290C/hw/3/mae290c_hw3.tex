...
\eeq
which is the first row of $\sL^{-1}\bu^n$. The pressure in the fractional method is determined by
\beq
p = \frac{\sL^{-1}\sD\bu^*}{\dt}\com
\eeq
which is, to $\mathcal{O}(\dt)$, the second row of $\sU^{-1}\sL^{-1}\bu^n$. The updated velocity is then the
 first row of $\sU^{-1}\sL^{-1}\bu^n$
 \beq
 \bu^{n+1} = \bu^* - \dt \sG p^{n+1} = \bu^* - \sG \sL^{-1}\sD\sA^{-1}\bu^*\per 
 \eeq

\item They are equivalent only to $\mathcal{O}(\dt)$.
