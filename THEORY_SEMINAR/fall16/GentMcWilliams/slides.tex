...
\onslide<2->\begin{block}{Along-isopynal eddy fluxes}
\beq
\overline{\mathbf{u}'\rho'}\cdot \nabla\bar{\rho} =
\overline{\mathbf{u}'_h\rho'}\cdot \nabla_h\bar{\rho} + \overline{w'\rho'}~\bar{\rho}_z\approx 0\per\nonumber
\eeq
\end{block}

\onslide<3->\begin{exampleblock}{The parameterization}
\beq
\textbf{Ansatz}:\quad\overline{\mathbf{u}_h'\rho'} = -\kappa \nabla_h \bar{\rho}\quad \Longrightarrow\quad
\overline{w'\rho'} = \kappa \frac{|\nabla_h \bar{\rho}|^2}{{\bar{\rho}}^2_z} \bar{\rho}_z
\per\nonumber
\eeq
\end{exampleblock}

\onslide<4->\begin{block}{Properties of the parameterization}
\begin{enumerate}
...
\end{block}
\begin{center}
{\includegraphics[width=.55\textwidth]{images/bci.png}}
\end{center}
\end{frame}
...
\beq\tag{G14}
\mathbf{A} = A^{mn}=\kappa\begin{bmatrix}
0 & 0 & - S_x\\
0 & 0 & - S_y\\
S_x & S_y & 0\\
\end{bmatrix}\com
...
\begin{frame}{GM + Redi}
  ...
  \beq
  (\p_t + \mathbf{u}\cdot \nabla)T = \underbrace{\p_m (K^{mn}\p_n T)}_{\text{Redi diffusion}}
   +  \underbrace{\p_m (A^{mn}\p_n T)}_{\text{GM stirring}}  \nonumber\per
  \eeq
  \end{block}
